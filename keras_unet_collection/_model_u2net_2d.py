...
def RSU(
    ...
    for i in range(depth):
        X = layers.concatenate(
            [X, X_skip[i]], axis=-1, name="{}_concat_{}".format(name, i)
        )
    ...
    X = layers.concatenate(
        [X, X_skip[depth]], axis=-1, name="{}_concat_out".format(name)
    )
    ...
    X = layers.add([X, X_skip[-1]], name="{}_out_add".format(name))
    return X
...
def RSU4F(
    ...
    X = layers.concatenate([X, X_skip[2]], axis=-1, name="{}_concat_out".format(name))
    X = CONV_stack(
        X,
        channel_out,
        kernel_size=3,
    ...
    return layers.add([X, X_skip[-1]], name="{}_out_add".format(name))
...
def u2net_2d(
    ...
    * Upsampling is achieved through bilinear interpolation and can be replaced by transpose convolutional layers.
    ...
    D = layers.concatenate(OUT_stack, axis=-1, name="{}_output_concat".format(name))
