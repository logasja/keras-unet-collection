...
def _crps_tf(y_true, y_pred, factor=0.05):
    ...
    dist = ops.math.reduce_std(y_pred)
...
def tversky_coef(y_true, y_pred, alpha=0.5, const=K.epsilon()):
    ...
    # get true pos (TP), false neg (FN), false pos (FP).
    true_pos = ops.reduce_sum(y_true_pos * y_pred_pos)
    false_neg = ops.reduce_sum(y_true_pos * (1 - y_pred_pos))
    false_pos = ops.reduce_sum((1 - y_true_pos) * y_pred_pos)
...
def iou_box_coef(y_true, y_pred, mode="giou", dtype="float32"):
    """
    Inersection over Union (IoU) and generalized IoU coefficients for bounding boxes.

    iou_box_coef(y_true, y_pred, mode='giou', dtype="float32")
...
def iou_box(y_true, y_pred, mode="giou", dtype="float32"):
    """
...
def iou_seg(y_true, y_pred, dtype="float32"):
    """
...
def triplet_1d(y_true, y_pred, N, margin=5.0):
    ...
    Input
    ----------
        y_true: a dummy input, not used within this function. Appeared as a requirment of ops.keras.loss function format.
